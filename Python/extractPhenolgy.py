...
class ExtPhen():
    def __init__(self,data,target,eDate):
        self.serTime = pd.to_datetime(data['time'],format = '%Y-%m-%d')

        self.serFit = data[target]
        try:
            self.eDate = pd.to_datetime (eDate,format = '%Y-%m-%d')
        except:
            print('eDate sould be in string format YYYY-MM-DD')
            sys.exit(1)
        
        # Check to esure serFit contais all Nan Values
    ...
    def getPhenMet(self,lookRng = 150, deltaVal = 0, isFreq = True, Lperc = 0.1, Uperc = 0.9):
        '''Calculates the peaks (maxima + minima) and fnd the seasonality metris'''
...
# Build up dataframe ---------------------------------------------------------------------------
            ...
            self.dfPhen = self.dfPhen.set_index(['time'])
...
if __name__ == '__main__':
    data = pd.read_csv('EE/GEE lite/Data/Imantha_TS.csv')
    ...
    print('\nEruption Phase : {}'.format(erupPhase))
